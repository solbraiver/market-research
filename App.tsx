...
const App: React.FC = () => {
  ...
  const renderCurrentStep = () => {
    if (isLoading) {
      ...
      if (currentStep === Step.PlanApproval) loadingText = "Webリサーチを実行し、レポートを生成しています... これには数分かかる場合があります。";
      return <Loader text={loadingText} />;
    }

    switch (currentStep) {
      case Step.ThemeSetting:
        return <Step1ThemeSettings onStart={handleStartAnalysis} initialMarket={targetMarket} initialCategory={offerCategory} />;
